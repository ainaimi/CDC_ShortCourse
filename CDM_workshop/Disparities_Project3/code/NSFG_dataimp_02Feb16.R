...
#######
#m<-subset(m,m$HISPRACE%in% c(2,3))
#######
...
strt<-Sys.time()
pc0<-pc1<-pTilde0<-pTilde1<-mod_boot<-iterboot1_mean<-iterboot1_var<-iterboot2_mean<-iterboot2_var<-NULL
for(i in 1:1000){
  i<-1
  set.seed(i*100)
  ## TAKE A SINGLE RE-SAMPLE FROM NSFG DATA WITH REPLACEMENT
  boot_id<-strata(m0,c("cs_clust"),size=resample_size,method=c("srswr"))
  m_boot<-getdata(imp_m,boot_id)
  nrow(m_boot);ncol(m_boot)
  
  ## IMPUTE THE SINGLE RESAMPLE
  imputation<-1;iteration<-5
  mr1<-subset(m_boot,m_boot$black==0)
  mr1$Prob<-mr1$Replicates<-mr1$ID_unit<-mr1$black<-mr1$Stratum<-NULL;nrow(mr1);ncol(mr1)
  mr2<-subset(m_boot,m_boot$black==1)
  mr2$Prob<-mr2$Replicates<-mr2$ID_unit<-mr2$black<-mr2$Stratum<-NULL;nrow(mr2);ncol(mr2)

  seed1<-i*123;seed2<-i*234
  ...
  exctract<-c("GETPRENA","csPNCAREWK","MATERNLV")
  ...
  imp1 <- complete(nsfg_imp1,"long");imp1$black<-0
  imp2 <- complete(nsfg_imp2,"long");imp2$black<-1
  
  nsfg_imp<-rbind(imp1,imp2)
...
#   min(nsfg_imp$PNCAREWK)
  dbootsub<-svydesign(id=~SECU, weight=~WGTQ1Q16, strata=~SEST,data=nsfg_imp,nest=T)
  
  ## TABLE FOR EDUCATION AND INCOME
  summary(svytable(~highschool+PAYDELIV,dbootsub))
  
  ## COMPUTE THE MEAN OF Y BY "BLACK" STATUS
  p_crude<-svyby(~breastfeed, ~black, dbootsub, svymean)
  ppc0<-as.matrix(p_crude$breastfeed)[1,1]
  ppc1<-as.matrix(p_crude$breastfeed)[2,1]
  pc0<-rbind(pc0,ppc0)
  pc1<-rbind(pc1,ppc1)
  ...
  propensity1<-svyglm(GETPRENA~csHPAGELB+csHPAGELB_s1+csHPAGELB_s2+
                        ...
                        black+highschool+PAYDELIV+
                        black*highschool+black*PAYDELIV+highschool*PAYDELIV,design=dbootsub,family=quasibinomial(link="logit"))
  summary(propensity1)
  
  nrow(nsfg_imp);nrow(as.matrix(propensity1$fitted.values))
  
  #nsfg_imp<-subset(nsfg_imp,nsfg_imp$HISPRACE%in% c(2,3))
  #nsfg_imp$pi_X1<-propensity1$fitted.values
  nsfg_imp$pi_X1<-propensity1$fitted.values
  
#   propensity2<-svyglm(PNCAREWK~csHPAGELB+csHPAGELB_s1+csHPAGELB_s2+
#                         csBFEEDWKSl1+csBFEEDWKSl1_s1+csBFEEDWKSl1_s2+
#                         breastfeedl1+breastfeedl1*csBFEEDWKSl1+
#                         PRIORSMK+POSTSMKS+PRIORSMK*POSTSMKS+PREGORDR+
#                         WANTRESP+FMARCON5+cs_ipi+cs_ipi_s1+cs_ipi_s2+
#                         cs_birthwgtl1+cs_birthwgtl1_s1+cs_birthwgtl1_s1+cd12+
#                         black+highschool+PAYDELIV+
#                         black*highschool+black*PAYDELIV+highschool*PAYDELIV,design=subset(dboot,nsfg_imp$GETPRENA==0),family=quasi(link="identity",variance="constant"))
#   summary(propensity2)
#   nsfg_imp$pi_X2<-ifelse(nsfg_imp$GETPRENA==0,0,propensity$fitted.values)
#   aggregate(nsfg_imp$pi_X2,list(nsfg_imp$GETPRENA),max)
  
## OUTPUT PROPENSITY SCORE PLOT DATA FROM A SINGLE RUN
#   pi_X<-as.data.frame(cbind(propensity$fitted.values,as.numeric(nsfg_imp$GETPRENA==5),nsfg_imp$black,nsfg_imp$highschool,nsfg_imp$PAYDELIV))
#   names(pi_X)<-c("pi_X","PNC","Black","HS","PayDeliv")  
#   head(pi_X)
#   library(Hmisc)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0),subset(pi_X$pi_X,pi_X$PNC==1),probability=T)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0&pi_X$Black==0),subset(pi_X$pi_X,pi_X$PNC==1&pi_X$Black==0),probability=T)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0&pi_X$Black==1),subset(pi_X$pi_X,pi_X$PNC==1&pi_X$Black==1),probability=T)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0&pi_X$HS==1),subset(pi_X$pi_X,pi_X$PNC==1&pi_X$HS==1),probability=T)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0&pi_X$HS==2),subset(pi_X$pi_X,pi_X$PNC==1&pi_X$HS==2),probability=T)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0&pi_X$PayDeliv==1),subset(pi_X$pi_X,pi_X$PNC==1&pi_X$PayDeliv==1),probability=T)
#   histbackback(subset(pi_X$pi_X,pi_X$PNC==0&pi_X$PayDeliv==2),subset(pi_X$pi_X,pi_X$PNC==1&pi_X$PayDeliv==2),probability=T)
#   write.table(pi_X, "/Users/Executive/Dropbox/Documents/Research/Papers/TestingFundamentalCause/PropensityPlots.txt", sep=",",row.names=FALSE)
  
  ## REDEFINE SAMPLING FRAME TO INCLUDE PS
  dboot<-svydesign(id=~SECU, weight=~WGTQ1Q16, strata=~SEST,data=nsfg_imp,nest=T)
  options(survey.lonely.psu = "adjust")
  ## ESTIMATE PNC EFFECT USING DOUBLY ROBUST METHOD, KEEP PNC PARAMETER ESTIMATES
  psi_RD<-as.matrix(coef(svyglm(breastfeed~GETPRENA+GETPRENA*black +GETPRENA*highschool+GETPRENA*PAYDELIV
                               +bs(pi_X1,df=4)+bs(pi_X1,df=4)*black +bs(pi_X1,df=4)*highschool+bs(pi_X1,df=4)*PAYDELIV
                                +black+highschool+PAYDELIV
                                ...
                                ))[c("GETPRENA","GETPRENA:black","GETPRENA:highschool","GETPRENA:PAYDELIV"),]  #
  X<-cbind(nsfg_imp$GETPRENA,nsfg_imp$GETPRENA*nsfg_imp$black,nsfg_imp$GETPRENA*nsfg_imp$highschool,nsfg_imp$GETPRENA*nsfg_imp$PAYDELIV) #
  names(nsfg_imp)
  table(nsfg_imp$GETPRENA)
  mean(X%*%psi_RD)
  
#   psi_RD<-svyglm(breastfeed~GETPRENA+GETPRENA*black
#                  +bs(pi_X1,df=4)+bs(pi_X1,df=4)*black
#                  +black+highschool+PAYDELIV
# #                  +csHPAGELB+csHPAGELB_s1+csHPAGELB_s2
# #                  +csBFEEDWKSl1+csBFEEDWKSl1_s1+csBFEEDWKSl1_s2
# #                  +breastfeedl1+breastfeedl1*csBFEEDWKSl1
...
#   summary(psi_RD)
  ...
  psi_RR<-as.matrix(coef(svyglm(breastfeed~GETPRENA+GETPRENA*black +GETPRENA*highschool+GETPRENA*PAYDELIV
                               +bs(pi_X1,df=4)+bs(pi_X1,df=4)*black +bs(pi_X1,df=4)*highschool+bs(pi_X1,df=4)*PAYDELIV
                               +black+highschool+PAYDELIV
                               +csHPAGELB+csHPAGELB_s1+csHPAGELB_s2
                               #+csBFEEDWKSl1+csBFEEDWKSl1_s1+csBFEEDWKSl1_s2
                               +breastfeedl1+breastfeedl1*csBFEEDWKSl1+breastfeedl1*csBFEEDWKSl1_s1+breastfeedl1*csBFEEDWKSl1_s2
                               +PRIORSMK+POSTSMKS+PRIORSMK*POSTSMKS+PREGORDR
                               +WANTRESP+FMARCON5+cs_ipi+cs_ipi_s1+cs_ipi_s2
                               +cs_birthwgtl1+cs_birthwgtl1_s1+cs_birthwgtl1_s1+cd12
                               +black*highschool+black*PAYDELIV+highschool*PAYDELIV
  ...
  p_trans<-svyby(~tildeY_RD, ~black, dboot, svymean)
  ppt0_RD<-as.matrix(p_trans$tildeY_RD)[1,1]
  ppt1_RD<-as.matrix(p_trans$tildeY_RD)[2,1]
  pTilde0_RD<-rbind(pTilde0_RD,ppt0_RD)
  pTilde1_RD<-rbind(pTilde1_RD,ppt1_RD)
  
  pc0;pTilde0;
  pc1;pTilde1;
  
  ## COMPUTE THE MEAN OF Y_TILDE BY "BLACK" STATUS UNDER MULTIPLICATIVE MODEL
  p_trans<-svyby(~tildeY_RR, ~black, dboot, svymean)
  ppt0_RR<-as.matrix(p_trans$tildeY_RR)[1,1]
  ppt1_RR<-as.matrix(p_trans$tildeY_RR)[2,1]
  pTilde0_RR<-rbind(pTilde0_RR,ppt0_RR)
  pTilde1_RR<-rbind(pTilde1_RR,ppt1_RR)
  
  #options(survey.lonely.psu = "adjust")
  #dboot<-subset(dboot,nsfg_imp$HISPRACE%in%c(2,3))
  
  summary(svyglm(tildeY_RD~black,design=dboot,family=quasi(link="identity",variance="constant")))
  summary(svyglm(breastfeed~black,design=dboot,family=quasi(link="identity",variance="constant")))
